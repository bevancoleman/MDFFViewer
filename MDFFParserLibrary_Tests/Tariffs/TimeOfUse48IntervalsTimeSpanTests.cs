...
[TestFixture]
public class TimeOfUse48IntervalsTimeSpanTests
{
    ...
    public void CheckGetRateTimeSpan()
    {
        ...
        Assert.That(_timeOfUse?.TariffNamePerInterval[10], Is.EqualTo(StrOffPeak)); // Uses Intervals
        Assert.That(_timeOfUse?.GetRate(new TimeSpan(5,0,0)), Is.EqualTo(0.30734m));    // uses TimeSpan
        
        Assert.That(_timeOfUse?.TariffNamePerInterval[12], Is.EqualTo(StrPeak));  // Uses Intervals
        Assert.That(_timeOfUse?.GetRate(new TimeSpan(6,0,0)), Is.EqualTo(0.44539m));  // uses TimeSpan
    }
